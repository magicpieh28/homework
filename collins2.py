...
class CollinsParser:
	...
	def parse(self, words: list, pos_tags: list):
				...
				for k in range(i+1, j):
					...
					for h_l in range(i, k):
						for h_r in range(k, j):
							...
							# left -> right
							l_score = self.getScore(pos_tags, span_l, span_r)
							span = CollinsSpan(i, j, h_l, l_score)
							h, score = self.addSpan(span)
							pair[h] = score

							# right -> left
							r_score = self.getScore(pos_tags, span_r, span_l)
	...
	def getScore(self, pos_tags, head, dep):
		# currently, use naive scoring function
		h_tag, d_tag = pos_tags[head.h], pos_tags[dep.h]
		if h_tag == "V" and d_tag == "N":
			score = 3.0
		elif h_tag == "N" and d_tag == "DT":
			score = 1.0
		elif h_tag == "N" and d_tag == "JJ":
			score = 1.0
		elif h_tag == "V" and d_tag == "PR":
			score = 2.0
		else:
			score = 0.1
